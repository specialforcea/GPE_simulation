...
for j=1:6
    grad = zeros(1,et(j)+1);
    grad(1:et(j)/2+1) = linspace(0,grads,et(j)/2+1);
    grad(et(j)/2+2:end) = linspace(grads,0,et(j)/2);
    draw = 0;
    phi = phi_e;
    ...
    %         plot(f./k_R*2*pi,sq(fourier_transform(phi,Nx,deltax)));
        ...
        save(strcat('simulation_results/02042019SOC_k_eigenstate/phi_1_25ms_318ms_grad_',num2str(j),'.mat'),'phi_1');
    end
    ...
    for i=1:320
        ...
        if draw==1 && mod(i,10)==0
            ...
            save(strcat('simulation_results/02042019SOC_k_eigenstate/spin125ms_25ms_318ms_grad_',num2str(j),'.mat'),'spin1');
            
            save(strcat('simulation_results/02042019SOC_k_eigenstate/width25ms_25ms_318ms_grad_',num2str(j),'.mat'),'width');
            save(strcat('simulation_results/02042019SOC_k_eigenstate/pos25ms_25ms_318ms_grad_',num2str(j),'.mat'),'pos');
