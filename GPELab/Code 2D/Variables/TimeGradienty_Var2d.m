...
%% INPUTS(OPTIONAL):
%%          Gradienty: Function or cell array of functions that depends on (t,X,Y) which will be multiplied by the gradient in the y direction of the wave function in the physical problem (function or cell array of functions)
...
function [Physics2D] = TimeGradienty_Var2d(Method, Physics2D, Gradienty, G)
%% Initializing the default gradient function
Default_Gradienty = @(t,X,Y) -1i*Physics2D.Omega*X; % Setting the default gradient function
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                ...
                if (G(n,m)~= 0)
                    ...
                    if(n == m)
                    Physics2D.TimeGradienty_function{n,m} = @(t,X,Y)G(n,m)*Default_Gradienty(t,X,Y); % Storing the gradient function as the correspondant value of G at the index added by the default gradient function
                    ...
                    Physics2D.TimeGradienty_function{n,m} = @(t,X,Y) 0; % Storing zero in the gradient function matrix
                    end
                % ELSEIF the value of the matrix at the index is zero
                elseif (G(n,m) == 0)
                    Physics2D.TimeGradienty_function{n,m} = @(X,Y) 0; % Storing zero in the gradient function matrix
                end
        end
        Physics2D.TimeGradienty_function_Index{n} = Gradienty_function_index; % Store the gradient function index for the 'm' index
    end
...
elseif (nargin == 4) && (iscell(Gradienty) == 0) && (isempty(Gradienty) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF the value of the matrix at the index is not zero
                if (G(n,m)~= 0)
                    Physics2D.TimeGradienty_function{n,m} = @(t,X,Y)G(n,m)*Gradienty(t,X,Y); % Storing the gradient function as the correspondant value of G at the index multiplied by the defined gradient function
                    Gradienty_function_index = [Gradienty_function_index,m]; % Add the 'm' index in the temporary gradient function index
                    Physics2D.Gradienty_compute_Index{m} = 1; % Setting the computation gradient index
                % ELSEIF the value of the matrix at the index is zero
                elseif (G(n,m) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                ...
                if (G(n,m)~= 0) && (isempty(Gradienty{n,m}) == 0)
                    Physics2D.TimeGradienty_function{n,m} = @(t,X,Y)G(n,m)*Gradienty{n,m}(t,X,Y); % Storing the gradient function as the correspondant value of G at the index multiplied by the defined gradient function
                    Gradienty_function_index = [Gradienty_function_index,m]; % Add the 'm' index in the temporary gradient function index
                    Physics2D.Gradienty_compute_Index{m} = 1; % Setting the computation gradient index
...
elseif (nargin == 3) && (iscell(Gradienty) == 0) && (isempty(Gradienty) == 1)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF it is a diagonal term
                if (n == m)
                    Physics2D.TimeGradienty_function{n,m} = @(t,X,Y)Default_Gradienty(t,X,Y); % Storing the gradient function as the default gradient function
                    Gradienty_function_index = [Gradienty_function_index,m]; % Add the 'm' index in the temporary gradient function index
                    Physics2D.Gradienty_compute_Index{m} = 1; % Setting the computation gradient index
...
elseif (nargin == 3) && (iscell(Gradienty) == 0) && (isempty(Gradienty) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF it is a diagonal term
                if (n == m)
                    Physics2D.TimeGradienty_function{n,m} = @(t,X,Y)Gradienty(t,X,Y); % Storing the gradient function as the defined gradient function
                    Gradienty_function_index = [Gradienty_function_index,m]; % Add the 'm' index in the temporary gradient function index
                    Physics2D.Gradienty_compute_Index{m} = 1; % Setting the computation gradient index
                % ELSEIF it is an extradiagonal term
                elseif (n ~= m)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
            ...
            if (isempty(Gradienty{n,m}) == 0)
                Physics2D.TimeGradienty_function{n,m} = @(t,X,Y)Gradienty{n,m}(t,X,Y); % Storing the gradient function as the defined gradient function
