...
%% INPUTS(OPTIONAL):
%%          Dispersion: Function or cell array of functions that depends on (t,FFTX,FFTY) which will be multiplied by the wave function in the physical problem (function or cell array of functions)
...
function [Physics2D] = TimeDispersion_Var2d(Method, Physics2D, TimeDispersion, G)
%% Initializing the default potential
Default_Dispersion = @(t,FFTX,FFTY) Physics2D.Delta*(FFTX.^2+FFTY.^2);
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                ...
                if (G(n,m)~= 0)
                    ...
                    if(n == m)
                    Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY)G(n,m)*Default_Dispersion(t,FFTX,FFTY); % Storing the potential as the correspondant value of G at the index added by the default potential
                    TimeDispersion_function_index = [TimeDispersion_function_index,m]; % Add the 'm' index in the temporary potential index
                    elseif (n ~= m)
                    Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY) 0; % Storing zero in the potential function matrix
                    end
        ...
        Physics2D.TimeDispersion_function_Index{n} = TimeDispersion_function_index; % Store the potential index for the 'm' index
    end
...
elseif (nargin == 4) && (iscell(TimeDispersion) == 0) && (isempty(TimeDispersion) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF the value of the matrix at the index is not zero
                if (G(n,m)~= 0)
                    Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY)G(n,m)*TimeDispersion(t,FFTX,FFTY); % Storing the potential as the correspondant value of G at the index multiplied by the defined potential
                    TimeDispersion_function_index = [TimeDispersion_function_index,m]; % Add the 'm' index in the temporary potential index
                % ELSEIF the value of the matrix at the index is zero
                elseif (G(n,m) == 0)
                    Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY)0; % Storing zero in the potential function matrix
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                ...
                if (G(n,m)~= 0) && (isempty(TimeDispersion{n,m}) == 0)
                    Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY)G(n,m)*TimeDispersion{n,m}(t,FFTX,FFTY); % Storing the potential as the correspondant value of G at the index multiplied by the defined potential
                    TimeDispersion_function_index = [TimeDispersion_function_index,m]; % Add the 'm' index in the temporary potential index
...
elseif (nargin == 3) && (iscell(TimeDispersion) == 0) && (isempty(TimeDispersion) == 1)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF it is a diagonal term
                if (n == m)
                    Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY)Default_Dispersion(t,FFTX,FFTY); % Storing the potential as the default potential
                    TimeDispersion_function_index = [TimeDispersion_function_index,m]; % Add the 'm' index in the temporary potential index
...
elseif (nargin == 3) && (iscell(TimeDispersion) == 0) && (isempty(TimeDispersion) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF it is a diagonal term
                if (n == m)
                    Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY)TimeDispersion(t,FFTX,FFTY); % Storing the potential as the defined potential
                    TimeDispersion_function_index = [TimeDispersion_function_index,m]; % Add the 'm' index in the temporary potential index
                % ELSEIF it is an extradiagonal term
                elseif (n ~= m)
                    Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY)0; % Storing zero in the potential function matrix
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
            ...
            if (isempty(TimeDispersion{n,m}) == 0)
                Physics2D.TimeDispersion_function{n,m} = @(t,FFTX,FFTY)TimeDispersion{n,m}(t,FFTX,FFTY); % Storing the potential as the defined potential
                TimeDispersion_function_index = [TimeDispersion_function_index,m]; % Add the 'm' index in the temporary potential index
