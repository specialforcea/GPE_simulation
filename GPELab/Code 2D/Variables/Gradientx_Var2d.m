...
%% INPUTS(OPTIONAL):
%%          Gradientx: Function or cell array of functions that depends on (X,Y) which will be multiplied by the gradient in the x direction of the wave function in the physical problem (function or cell array of functions)
...
function [Physics2D] = Gradientx_Var2d(Method, Physics2D, Gradientx, G)
%% Initializing the default gradient function
Default_Gradientx = @(X,Y) 1i*Physics2D.Omega*Y;
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                ...
                if (G(n,m)~= 0)
                    ...
                    if(n == m)
                    Physics2D.Gradientx_function{n,m} = @(X,Y)G(n,m)*Default_Gradientx(X,Y); % Storing the gradient function as the correspondant value of G at the index added by the default gradient function
                    ...
                    Physics2D.Gradientx_function{n,m} = @(X,Y) 0; % Storing zero in the gradient function matrix
                    end
        ...
        Physics2D.Gradientx_function_Index{n} = Gradientx_function_index; % Store the gradient function index for the 'm' index
    end
...
elseif (nargin == 4) && (iscell(Gradientx) == 0) && (isempty(Gradientx) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF the value of the matrix at the index is not zero
                if (G(n,m)~= 0)
                    Physics2D.Gradientx_function{n,m} = @(X,Y)G(n,m)*Gradientx(X,Y); % Storing the gradient function as the correspondant value of G at the index multiplied by the defined gradient function
                    Gradientx_function_index = [Gradientx_function_index,m]; % Add the 'm' index in the temporary gradient function index
                    Physics2D.Gradientx_compute_Index{m} = 1; % Setting the computation gradient index
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                ...
                if (G(n,m)~= 0) && (isempty(Gradientx{n,m}) == 0)
                    Physics2D.Gradientx_function{n,m} = @(X,Y)G(n,m)*Gradientx{n,m}(X,Y); % Storing the gradient function as the correspondant value of G at the index multiplied by the defined gradient function
                    Gradientx_function_index = [Gradientx_function_index,m]; % Add the 'm' index in the temporary gradient function index
                    Physics2D.Gradientx_compute_Index{m} = 1; % Setting the computation gradient index
...
elseif (nargin == 3) && (iscell(Gradientx) == 0) && (isempty(Gradientx) == 1)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF it is a diagonal term
                if (n == m)
                    Physics2D.Gradientx_function{n,m} = @(X,Y)Default_Gradientx(X,Y); % Storing the gradient function as the default gradient function
                    Gradientx_function_index = [Gradientx_function_index,m]; % Add the 'm' index in the temporary gradient function index
                    Physics2D.Gradientx_compute_Index{m} = 1; % Setting the computation gradient index
...
elseif (nargin == 3) && (iscell(Gradientx) == 0) && (isempty(Gradientx) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF it is a diagonal term
                if (n == m)
                    Physics2D.Gradientx_function{n,m} = @(X,Y)Gradientx(X,Y); % Storing the gradient function as the defined gradient function
                    Gradientx_function_index = [Gradientx_function_index,m]; % Add the 'm' index in the temporary gradient function index
                    Physics2D.Gradientx_compute_Index{m} = 1; % Setting the computation gradient index
                % ELSEIF it is an extradiagonal term
                elseif (n ~= m)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
            ...
            if (isempty(Gradientx{n,m}) == 0)
                Physics2D.Gradientx_function{n,m} = @(X,Y)Gradientx{n,m}(X,Y); % Storing the gradient function as the defined gradient function
