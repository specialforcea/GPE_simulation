...
function [Phi_out] = operator_Full_RSP_PLaplacian2d(Phi_in, Psi, FFTPsi, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics2D.TimeDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + ifft(FFTPhysics2D.TimeDispersionImp{n,m}.*fft2(Phi{m}))/2;
        end
    end
    ...
    GPE_Phi = Phi{n} + ifft2((1/Method.Deltat + FFTPhysics2D.Dispersion{n,n}/2 + FFTPhysics2D.TimeDispersionImp{n,n}/2 + FFTPhysics2D.StochasticDispersion{n,n}/2).^(-1).*fft2(GPE_Phi)); % Computing the IFFT of the wave function with the operators applied then apply the Laplace preconditioner
