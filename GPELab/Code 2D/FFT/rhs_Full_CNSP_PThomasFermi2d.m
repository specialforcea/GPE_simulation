...
function [Phi_out] = rhs_Full_CNSP_PThomasFermi2d(Phi_in, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics2D.FFTNonlinearity_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.Beta*FFTPhysics2D.FFTNonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    ...
    GPE_Phi = GPE_Phi + 1/Method.Deltat*Phi{n} ...
        + ifft2((FFTPhysics2D.Delta*(FFTOperators2D.Dx+FFTOperators2D.Dy)/2 - FFTPhysics2D.Dispersion{n,n}/2).*fft2(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
    GPE_Phi = 1./(1/Method.Deltat + FFTPhysics2D.Potential{n,n}/2+FFTPhysics2D.TimePotential{n,n}/2+ FFTPhysics2D.Beta*FFTPhysics2D.Nonlinearity{n,n}/2 + FFTPhysics2D.Beta*FFTPhysics2D.FFTNonlinearity{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
