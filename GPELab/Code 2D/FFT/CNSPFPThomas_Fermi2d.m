%% Computing the Full ThomasFermi preconditioner
%% INPUTS:
...
function FPTHomasFermi = CNSPFPThomas_Fermi2d(Method, FFTPhysics2D, FFTGeometry2D)
%% Initialization of the potential and nonlinear matricial operators 
PotNLMat = cell(Method.Ncomponents);
for n = 1:Method.Ncomponents
    for m = 1:Method.Ncomponents
        PotNLMat{n,m} = (n == m)*1/Method.Deltat + FFTPhysics2D.Potential{n,m}/2 + FFTPhysics2D.TimePotential{n,m}/2 + FFTPhysics2D.Beta*FFTPhysics2D.Nonlinearity{n,m}/2 + FFTPhysics2D.Beta*FFTPhysics2D.FFTNonlinearity{n,m}/2;
    end
end
