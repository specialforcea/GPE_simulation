%% Solving RSP-DS scheme on a single step of time using a direct iterative method with Laplace preconditionner
%% INPUTS:
...
function [Phi_out, Psi, FFTPsi, iter] = operator_Partial_RSP2d(Phi_in, Psi, FFTPsi, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
while (Phi_evo >Method.Iterative_tol)&&(iter<Method.Iterative_maxit)
    ...
    RHS = rhs_Partial_RSP_PLaplacian2d(Phi_in, Psi, FFTPsi, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D); % Computing rhs
%% Computing gradients
% FOR each component
for n = 1:Method.Ncomponents
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics2D.Potential_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.Potential{n,m}.*Phi_tmp{m}/2; % Computing the wave function with the potential operator applied
    end
    % FOR each component where the nonlinearity is non null
    for m = FFTPhysics2D.Nonlinearity_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.Beta*Psi{n,m}.*Phi_tmp{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
     % FOR each component where the nonlinearity is non null
    for m = FFTPhysics2D.FFTNonlinearity_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.Beta*FFTPsi{n,m}.*Phi_tmp{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics2D.TimePotential_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.TimePotential{n,m}.*Phi_tmp{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    ...
    for m = FFTPhysics2D.Dispersion_function_Index{n}
        ...
        GPE_Phi = GPE_Phi + alpha(n).*Phi_tmp{n}/2; % Computing the wave function with the potential and nonlinear operators applied
        Phi_tmp{n} = ifft2(1./(1/Method.Deltat+alpha(n)/2-FFTPhysics2D.Delta/2*(FFTOperators2D.Dx+FFTOperators2D.Dy) + FFTPhysics2D.Dispersion{n,n}).*fft2(RHS{n} + GPE_Phi));  % Computing the IFFT of the explicit and implicit wave function then apply the Laplace preconditionner
