...
function [Phi_out] = operator_Full_RSP_PThomasFermi2d(Phi_in, Psi, FFTPsi, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics2D.TimeGradientx_function_Index{n}
        GPE_Phi = GPE_Phi + FFTPhysics2D.TimeGradientx{n,m}.*Gradx_Phi{m}/2; % Computing the wave function with the gradient operators in the x direction applied
    end
    % FOR each component where the gradient in the y direction is non null
    for m = FFTPhysics2D.TimeGradienty_function_Index{n}
        GPE_Phi = GPE_Phi + FFTPhysics2D.TimeGradienty{n,m}.*Grady_Phi{m}/2; % Computing the wave function with the gradient operators in the y direction applied
    end
    ...
    GPE_Phi = GPE_Phi + ifft2((FFTPhysics2D.Dispersion{n,n}/2 + FFTPhysics2D.TimeDispersionImp{n,n}/2 + FFTPhysics2D.StochasticDispersion{n,n}/2).*fft2(Phi{n})); % Computing the wave function with the Laplacian operator applied
    GPE_Phi = Phi{n} + 1./(1/Method.Deltat + FFTPhysics2D.Potential{n,n}/2+ FFTPhysics2D.TimePotentialImp{n,n}/2+ FFTPhysics2D.StochasticPotential{n,n}/2+ FFTPhysics2D.Beta*Psi{n,n}/2+ FFTPhysics2D.Beta*FFTPsi{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
