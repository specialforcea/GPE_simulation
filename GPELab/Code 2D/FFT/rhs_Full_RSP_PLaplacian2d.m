...
function [Phi_out] = rhs_Full_RSP_FPLaplacian2d(Phi_in, Psi, FFTPsi, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = (1/Method.Deltat)*Phi{n} + GPE_Phi;
    GPE_Phi = ifft2((1/Method.Deltat + FFTPhysics2D.Dispersion{n,n}/2 + FFTPhysics2D.TimeDispersionImp{n,n}/2 + FFTPhysics2D.StochasticDispersion{n,n}/2).^(-1).*fft2(GPE_Phi)); % Computing the IFFT of the wave function with the operators applied then apply the Laplace preconditioner
    Phi_out(:,(1+(n-1)*FFTGeometry2D.Nx):(n*FFTGeometry2D.Nx)) = GPE_Phi; % Storing the wave function with the operators and the preconditioner applied
end
