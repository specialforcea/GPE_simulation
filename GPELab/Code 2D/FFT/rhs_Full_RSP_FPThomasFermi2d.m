%% Applying the operators corresponding to the explicit part of the RSP scheme with the Thomas-Fermi preconditioner
%% INPUTS:
...
function [Phi_out] = rhs_Full_RSP_FPThomasFermi2d(Phi_in, Psi, FFTPsi, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics2D.Gradientx_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.Gradientx{n,m}.*Gradx_Phi{m}/2; % Computing the wave function with the gradient operators in the x direction applied
    end
    % FOR each component where the gradient in the y direction is non null
    for m = FFTPhysics2D.Gradienty_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.Gradienty{n,m}.*Grady_Phi{m}/2; % Computing the wave function with the gradient operators in the y direction applied
    end
    % FOR each component where the gradient in the x direction is non null
    for m = FFTPhysics2D.TimeGradientx_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.TimeGradientxExp{n,m}.*Gradx_Phi{m}/2; % Computing the wave function with the gradient operators in the x direction applied
    end
    % FOR each component where the gradient in the y direction is non null
    for m = FFTPhysics2D.TimeGradienty_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.TimeGradientyExp{n,m}.*Grady_Phi{m}/2; % Computing the wave function with the gradient operators in the y direction applied
    end
    % FOR each component where the potential is non null
    for m = FFTPhysics2D.Potential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.Potential{n,m}.*Phi{m}/2; % Computing the wave function with the potential operator applied
    end
    % FOR each component where the nonlinearity is non null
    for m = FFTPhysics2D.Nonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.Beta*Psi{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the nonlinearity is non null
    for m = FFTPhysics2D.FFTNonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.Beta*FFTPsi{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics2D.TimePotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.TimePotentialExp{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics2D.StochasticPotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.StochasticPotential{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    ...
    for m = FFTPhysics2D.Dispersion_function_Index{n}
        ...
        if (m ~= n)
            GPE_Phi{n} = GPE_Phi{n} - ifft2(FFTPhysics2D.Dispersion{n,m}.*fft2(Phi{m}))/2; % Computing the wave function with the dispersion operator applied
        end
    end
     % FOR each component where the dipersion is non null
    for m = FFTPhysics2D.TimeDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
            GPE_Phi{n} = GPE_Phi{n} - ifft2(FFTPhysics2D.DispersionExp{n,m}.*fft2(Phi{m}))/2; % Computing the wave function with the dispersion operator applied
        end
    end
    ...
    for m = FFTPhysics2D.StochasticDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
            GPE_Phi{n} = GPE_Phi{n} - ifft2(FFTPhysics2D.StochasticDispersion{n,m}.*fft2(Phi{m}))/2; % Computing the wave function with the stochastic dispersion operator applied
        end
    end
    GPE_Phi{n} = GPE_Phi{n} + 1/Method.Deltat*Phi{n} ...
        + ifft2((- FFTPhysics2D.Dispersion{n,n}/2 - FFTPhysics2D.TimeDispersionExp{n,n}/2 - FFTPhysics2D.StochasticDispersion{n,n}/2).*fft2(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
end
...
for n = 1:Method.Ncomponents
    ...
    for m = 1:Method.Ncomponents
        Phi = Phi + FFTPhysics2D.FPThomasFermi{n,m}.*GPE_Phi{m}; % Applying the Thomas-Fermi preconditioner
    end
    Phi_out(:,(1+(n-1)*FFTGeometry2D.Nx):(n*FFTGeometry2D.Nx)) = Phi; % Storing the wave function with the operators applied
end
