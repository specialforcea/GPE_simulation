%% Applying the operators corresponding to the explicit part of the CNSP-CNFG scheme with the Thomas-Fermi preconditioner
%% INPUTS:
...
function [Phi_out] = rhs_Full_CNSP_FPThomasFermi2d(Phi_in, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics2D.Nonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.Beta*FFTPhysics2D.Nonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the nonlinearity is non null
    for m = FFTPhysics2D.FFTNonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.Beta*FFTPhysics2D.FFTNonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics2D.TimePotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics2D.TimePotential{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
