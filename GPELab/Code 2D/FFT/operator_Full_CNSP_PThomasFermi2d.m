...
function [Phi_out] = operator_Full_CNSP_PThomasFermi2d(Phi_in, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = GPE_Phi + ifft2(FFTPhysics2D.Dispersion{n,n}/2.*fft2(Phi{n})); % Computing the wave function with the Laplacian operator applied
    GPE_Phi = Phi{n} + 1./(1/Method.Deltat + FFTPhysics2D.Potential{n,n}/2+ FFTPhysics2D.TimePotential{n,n}/2+ FFTPhysics2D.Beta*FFTPhysics2D.Nonlinearity{n,n}/2+ FFTPhysics2D.Beta*FFTPhysics2D.FFTNonlinearity{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
