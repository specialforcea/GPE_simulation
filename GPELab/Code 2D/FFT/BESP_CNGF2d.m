...
function [Phi_final, Outputs] = BESP_CNGF2d(Phi_0, Method, Geometry2D, Physics2D, Outputs, Print, Figure)
...
while (Method.EvolutionCriterion > Method.Stop_crit{2}*Method.Deltat)&&(Method.Iterations<Method.Max_iter)
    ...
    elseif strcmp(Method.Stop_crit{1},'Function')
        if (Method.Iterations ~= 1)
            Function_eval_tmp = Function_eval;
            Function_eval = sqrt(FFTGeometry2D.dx*FFTGeometry2D.dy*sum(sum(Method.Stop_crit{3}(FFTPhi, -1i*FFTOperators2D.Gx, -1i*FFTOperators2D.Gy, FFTGeometry2D.X, FFTGeometry2D.Y)))); % Computing the energy of each wave function
            Method.EvolutionCriterion = abs(Function_eval-Function_eval_tmp);
        else
            Function_eval = 0;
        end
    end
