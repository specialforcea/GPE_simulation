...
function [Phi_out] = operator_Full_RSP_FPThomasFermi2d(Phi_in, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics2D.TimeGradientx_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} + FFTPhysics2D.TimeGradientxImp{n,m}.*Gradx_Phi{m}/2; % Computing the wave function with the gradient operators in the x direction applied
    end
    % FOR each component where the gradient in the y direction is non null
    for m = FFTPhysics2D.TimeGradienty_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} + FFTPhysics2D.TimeGradientyImp{n,m}.*Grady_Phi{m}/2; % Computing the wave function with the gradient operators in the y direction applied
    end
    ...
    for m = FFTPhysics2D.TimeDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
            GPE_Phi{n} = GPE_Phi{n} + ifft2(FFTPhysics2D.TimeDispersionImp{n,m}.*fft2(Phi{m}))/2; % Computing the wave function with the dispersion operator applied
        end
    end
    ...
    GPE_Phi{n} = GPE_Phi{n} + ifft2((FFTPhysics2D.Dispersion{n,n}/2 + FFTPhysics2D.TimeDispersionImp{n,n}/2 +  FFTPhysics2D.StochasticDispersion{n,n}/2).*fft2(Phi{n})); % Computing the wave function with the Laplacian operator applied
end
...
for n = 1:Method.Ncomponents
   Phi = zeros(FFTGeometry2D.Ny,FFTGeometry2D.Nx); % Initialization of the wave function
    ...
    Phi_out(:,(1+(n-1)*FFTGeometry2D.Nx):(n*FFTGeometry2D.Nx)) = Phi; % Storing the wave function with the operators and the preconditioner applied
end
