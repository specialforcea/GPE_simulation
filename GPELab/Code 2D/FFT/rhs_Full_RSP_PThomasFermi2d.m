...
function [Phi_out] = rhs_Full_RSP_PThomasFermi2d(Phi_in, Psi, FFTPsi, Method, FFTGeometry2D, FFTPhysics2D, FFTOperators2D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics2D.FFTNonlinearity_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.Beta*FFTPsi{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics2D.TimePotential_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics2D.TimePotentialExp{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    ...
    for m = FFTPhysics2D.TimeDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
            GPE_Phi = GPE_Phi - ifft2(FFTPhysics2D.TimeDispersionExp{n,m}.*fft2(Phi{m}))/2; % Computing the wave function with the dispersion operator applied
        end
    end
    ...
    GPE_Phi = 1./(1/Method.Deltat + FFTPhysics2D.Potential{n,n}/2+FFTPhysics2D.TimePotentialImp{n,n}/2+FFTPhysics2D.StochasticPotential{n,n}/2+ FFTPhysics2D.Beta*Psi{n,n}/2 + FFTPhysics2D.Beta*FFTPsi{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
