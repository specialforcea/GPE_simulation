%% Crank Nicholson Finite Differences for the Dynamic System (CNFD-DS) : Semi-implicit FD scheme for the computation of the dynamic Gross Pitaevskii equation
%% INPUTS:
...
%% OUTPUTS:
%%          Phi_final: Dynamic's wave functions computated with the CNFD method (cell array)
%%          Outputs: Different outputs computed during the computation of the problem (structure) (see OutputsINI_Var2D.m and FFTOutputs_Var2d.m)
%% FUNCTIONS USED:
...
%%          Local_Krylov_CNFD_solution2d: To compute the CNFG on a single time step using the iterative CNFD method (line 57)
%%          Local_Direct_CNFD_solution2d: To compute the CNFG on a single time step using the direct CNFD method (line 63)
...
function [Phi_final, Outputs] = CNFD_DS2d(Phi_0, Method, Geometry2D, Physics2D, Outputs, Print, Figure)
...
%% Computation of the dynamic GPE via CNFD
