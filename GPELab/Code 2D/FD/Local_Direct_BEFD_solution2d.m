%% Computation of a step in time using a direct inversion with BEFD scheme
%% INPUTS:
...
function Phi = Local_Direct_BEFD_solution2d(Phi, Method, FDGeometry2D, FDPhysics2D, FDOperators2D)

%% Computing the explicit nonlinearity and time-dependent potential
% FOR each component
...
%% Computing the wave functions after a single time step using a direct inversion for BEFD
%If the computation is dynamic
if (strcmp(Method.Computation,'Dynamic'))
    Method.Deltat = 1i*Method.Deltat;
end
M = (1/Method.Deltat)*FDOperators2D.Id + FDOperators2D.Linear_Operators + FDPhysics2D.Beta*FDOperators2D.Nonlinearity_matrix + FDOperators2D.TimePotential_matrix; % Computation of the system
Phi_vect = M\(Phi_vect)*Method.Deltat; % Direct inversion of the system
