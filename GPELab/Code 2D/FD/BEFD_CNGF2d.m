%% Backward Euler Finite Differences for the Continuous Normalized Gradient Flow (BEFD-CNFG) : Semi-implicit FD scheme for the computation of grounds states of the Gross Pitaevskii equation
%% INPUTS:
...
%% OUTPUTS:
%%          Phi_final: Groundstate's wave functions computated with the BEFD method (cell array)
%%          Outputs: Different outputs computed during the computation of the ground states (structure) (see OutputsINI_Var2D.m and FFTOutputs_Var2d.m)
%% FUNCTIONS USED:
...
%%          Local_Krylov_BEFD_solution2d: To compute the CNFG on a single time step using the iterative BEFD method (line 63)
%%          Local_Direct_BEFD_solution2d: To compute the CNFG on a single time step using the direct BEFD method (line 69)
...
function [Phi_final, Outputs] = BEFD_CNGF2d(Phi_0, Method, Geometry2D, Physics2D, Outputs, Print, Figure)
...
%% Computation of the ground state via BEFD
...
while (Method.EvolutionCriterion > Method.Stop_crit{2}*Method.Deltat)&&(Method.Iterations<Method.Max_iter)
    ...
    Energy = Energy_GPE_Diff2d(FDPhi, Method, FDGeometry2D, FDPhysics2D, FDOperators2D); % Computing the energy of each wave function
