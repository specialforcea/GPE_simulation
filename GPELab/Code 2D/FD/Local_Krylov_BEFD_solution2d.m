%% Computation of a step in time using an iterative Krylov method with BEFD scheme
%% INPUTS:
...
function [Phi, flag, relres, iter, resvec] = Local_Krylov_BEFD_solution2d(Phi, Method, FDGeometry2D, FDPhysics2D, FDOperators2D)
...
%% Computing the wave functions after a single time step using Krylov iterative method for BEFD
...
if (strcmp(Method.Precond,'Laplace') == 1)
   ...
   % Laplace preconditioner for the implicit part of the BEFD scheme 
   ...
   [Phi_out, flag, relres, iter, resvec] = bicgstab(((1/Method.Deltat)*FDOperators2D.Id + FDOperators2D.Linear_Operators + FDPhysics2D.Beta*FDOperators2D.Nonlinearity_matrix + FDOperators2D.TimePotential_matrix),...
...
elseif (strcmp(Method.Precond,'ThomasFermi') == 1)
   ...
   % Moreover, it uses the initial components' wave functions as initial vector.
   PThomasFermi_matrix = inv((1/Method.Deltat)*FDOperators2D.Id + FDOperators2D.Potential_matrix + FDOperators2D.TimePotential_matrix + FDPhysics2D.Beta*FDOperators2D.Nonlinearity_matrix);
   [Phi_out, flag, relres, iter, resvec] = bicgstab((FDOperators2D.Id + PThomasFermi_matrix*(-FDPhysics2D.Delta*FDOperators2D.Laplacian + FDOperators2D.Gradientx_matrix + FDOperators2D.Gradienty_matrix)),...
   (1/Method.Deltat)*PThomasFermi_matrix*Phi_vect, Method.Iterative_tol, Method.Iterative_maxit,[],[],Phi_vect);
