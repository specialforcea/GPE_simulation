...
%%          save: Variable containing the information wherever to save or not the solution during the simulation
%%          outputs: User defined functions for each component which depend on (phi,X,Y,Z) to be stored and printed in the informations (cell array of functions)
...
function [Outputs] = OutputsINI_Var3d(Method, Evo_outputs, save, userdef_outputs, userdef_outputs_names, globaluserdef_outputs, globaluserdef_outputs_names)
%% Initialization of the Outputs variables
    ...
    Outputs.z_rms = cell(1,Method.Ncomponents); % Initialization of the variable containing the root mean square in the z direction
...
%% Initialization of the user defined outputs
    ...
    Outputs.User_defined_local = cell(Method.Ncomponents,length(userdef_outputs)); % Initialization of the variable containing the user defined functions
    Outputs.User_defined_function_local = userdef_outputs;
    ...
    % ELSE if there are no names for the user defined outputs functions
        ...
        for n = 1:Outputs.User_defined_number
            Outputs.User_defined_names_local{n} = strcat('User defined function',32,num2str(n)); % Storing the default name
        end
    end
% ELSE if there are no user defined outputs functions
else
    Outputs.User_compute_local = 0; % Setting not to compute user defined functions
end
...
%% Initialization of the user defined outputs
    ...
    % ELSE if there are no names for the user defined outputs functions
        ...
        % FOR each user defined function
        for n = 1:Outputs.User_defined_number
            Outputs.User_defined_names_global{n} = strcat('User defined function',32,num2str(n)); % Storing the default name
        end
    end
% ELSE if there are no user defined outputs functions
else
    Outputs.User_compute_global = 0; % Setting not to compute user defined functions
end