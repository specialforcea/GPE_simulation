%% Applying the operators corresponding to the explicit part of the RSP scheme with the Thomas-Fermi preconditionner
%% INPUTS:
...
function [Phi_out] = rhs_Full_RSP_FPThomasFermi3d(Phi_in, Psi, FFTPsi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
% FOR each component
for n = 1:Method.Ncomponents
    GPE_Phi{n} = zeros(FFTGeometry3D.Ny,FFTGeometry3D.Nx,FFTGeometry3D.Nz); % Initializing the variable that will contain the wave function with the operators applied
    % FOR each component where the gradient in the x direction is non null
    for m = FFTPhysics3D.Gradientx_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.Gradientx{n,m}.*Gradx_Phi{m}/2; % Computing the wave function with the gradient operators in the x direction applied
    end
    % FOR each component where the gradient in the y direction is non null
    for m = FFTPhysics3D.Gradienty_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.Gradienty{n,m}.*Grady_Phi{m}/2; % Computing the wave function with the gradient operators in the y direction applied
    end
    % FOR each component where the gradient in the z direction is non null
    for m = FFTPhysics3D.Gradientz_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.Gradientz{n,m}.*Gradz_Phi{m}/2; % Computing the wave function with the gradient operators in the z direction applied
    end
    % FOR each component where the potential is non null
    for m = FFTPhysics3D.Potential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.Potential{n,m}.*Phi{m}/2; % Computing the wave function with the potential operator applied
    end
    % FOR each component where the nonlinearity is non null
    for m = FFTPhysics3D.Nonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.Beta*Psi{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the non-local nonlinearity is non null
    for m = FFTPhysics3D.Nonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.Beta*FFTPsi{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics3D.TimePotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.TimePotentialExp{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    % FOR each component where the stochastic potential is non null
    for m = FFTPhysics3D.StochasticPotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.StochasticPotential{n,m}.*Phi{m}/2; % Computing the wave function with the stochastic potential operator applied
    end
    ...
    for m = FFTPhysics3D.StochasticDispersion_function_Index{n}
        ...
        if (m ~= n)
            GPE_Phi{n} = GPE_Phi{n} - ifftn(FFTPhysics3D.StochasticDispersion{n,m}.*fftn(Phi{m}))/2; % Computing the wave function with the stochastic dispersion applied
        end
    end
    ...
    for m = FFTPhysics3D.Dispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
            GPE_Phi{n} = GPE_Phi{n} - ifftn(FFTPhysics3D.Dispersion{n,m}.*fftn(Phi{m}))/2; % Computing the wave function with the dispersion operator applied
        end 
    end
    % FOR each component where the dispersion is non null
    for m = FFTPhysics3D.TimeDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
            GPE_Phi{n} = GPE_Phi{n} - ifftn(FFTPhysics3D.TimeDispersionExp{n,m}.*fftn(Phi{m}))/2; % Computing the wave function with the dispersion operator applied
        end 
    end
    GPE_Phi{n} = GPE_Phi{n} + 1/Method.Deltat*Phi{n} ...
        + ifftn(( - FFTPhysics3D.Dispersion{n,n}/2 - FFTPhysics3D.TimeDispersionExp{n,n}/2 - FFTPhysics3D.StochasticDispersion{n,n}/2).*fftn(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
    GPE_Phi{n} = 1./(1/Method.Deltat + FFTPhysics3D.Potential{n,n}/2 + FFTPhysics3D.TimePotentialImp{n,n}/2 + FFTPhysics3D.StochasticPotential{n,n}/2 + FFTPhysics3D.Beta*Psi{n,n}/2 + FFTPhysics3D.Beta*FFTPsi{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
end

%% Applying the full Thomas Fermi preconditioner
for n = 1:Method.Ncomponents
    Phi = zeros(FFTGeometry3D.Ny,FFTGeometry3D.Nx,FFTGeometry3D.Nz); % Initialization of the wave function
    for m = 1:Method.Ncomponents
        Phi = Phi + FFTPhysics3D.FPThomasFermi{n,m}.*GPE_Phi{m}; % Applying the Thomas-Fermi preconditioner
    end
    Phi_out(:,:,(1+(n-1)*FFTGeometry3D.Nz):(n*FFTGeometry3D.Nz)) = Phi; % Storing the wave function with the operators applied
end
