%% Applying the operators corresponding to the implicit part of the RSP scheme with the Laplace preconditioner
%% INPUTS:
...
function [Phi_out] = operator_Full_RSP_FPLaplacian3d(Phi_in, Psi, FFTPsi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    Phi_out(:,:,(1+(n-1)*FFTGeometry3D.Nz):(n*FFTGeometry3D.Nz)) = Phi{n} + ifftn(FFTPhi); % Storing the wave function with the operators and the preconditionner applied
end
