...
function [Phi_final, Outputs] = BESP_DS3d(Phi_0, Method, Geometry3D, Physics3D, Outputs, Print, Figure)
...
while (Method.Iterations*Method.Deltat<Method.Stop_time)
    ...
    %% Computing ground state using the full BESP method  
        [FFTPhi, flag, relres, iter, resvec] = Local_Full_BESP_solution3d(FFTPhi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D); % Computation of the ground state using the full BESP-CNFG method on a single step of time
    ...
    %% Computing ground state using the partial BESP method (paper by Zeng-Zhang (Eq. 3.19)    
        [FFTPhi, iter] = Local_Partial_BESP_solution3d(FFTPhi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D); % Computation of the ground state using the partial BESP-CNFG method on a single step of time
        Outputs.iteration_vec(Method.Iterations) = iter; % Storing total number of BICGSTAG iterations at the iteration-th iteration of the Gradient Flow  
    end
