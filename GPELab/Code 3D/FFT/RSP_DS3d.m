%% Relaxation SPectral for the Dynamic System (RSP-DS) : Semi-implicit FFT scheme for the computation of the dynamic Gross Pitaevskii equation
%% INPUTS:
...
function [Phi_final, Outputs] = RSP_DS3d(Phi_0, Method, Geometry3D, Physics3D, Outputs, Print, Figure)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.Nonlinearity_function_Index{n}
        Psi{n,m} = FFTPhysics3D.Nonlinearity_function{n,m}(FFTPhi,FFTGeometry3D.X,FFTGeometry3D.Y,FFTGeometry3D.Z); % Initializing the relaxation variable corresponding to the nonlinearity
    end
end
for n = 1:Method.Ncomponents
    % FOR each component
    for m = FFTPhysics3D.FFTNonlinearity_function_Index{n}
        FFTPsi{n,m} = FFTPhysics3D.FFTNonlinearity_function{n,m}(FFTPhi,FFTGeometry3D.X,FFTGeometry3D.Y,FFTGeometry3D.Z,-1i*FFTOperators3D.Gx,-1i*FFTOperators3D.Gy,-1i*FFTOperators3D.Gz); % Initializing the relaxation variable corresponding to the non local nonlinearity
    end
end
...
while (Method.Iterations*Method.Deltat<Method.Stop_time)
    ...
    %% Computing ground state using the full CNSP method  
        [FFTPhi, Psi, FFTPsi, flag, relres, iter, resvec] = Local_Full_RSP_solution3d(FFTPhi, Psi, FFTPsi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D); % Computation of the ground state using the CNSP-CNFG method on a single step of time
