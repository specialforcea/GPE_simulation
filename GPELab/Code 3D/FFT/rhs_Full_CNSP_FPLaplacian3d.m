%% Applying the operators corresponding to the explicit part of the BESP-CNFG scheme with the Thomas-Fermi preconditionner
%% INPUTS:
...
function [Phi_out] = rhs_Full_CNSP_FPLaplacian3d(Phi_in, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.Nonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.Beta*FFTPhysics3D.Nonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the non-local nonlinearity is non null
    for m = FFTPhysics3D.Nonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.Beta*FFTPhysics3D.FFTNonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics3D.TimePotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics3D.TimePotential{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    ...
    FFTGPE_Phi{n} = fftn(GPE_Phi{n} + 1/Method.Deltat*Phi{n});
end

%% Applying the Thomas Fermi preconditioner
for n = 1:Method.Ncomponents
    FFTPhi_tmp = zeros(FFTGeometry3D.Ny,FFTGeometry3D.Nx,FFTGeometry3D.Nz);
    for m = 1:Method.Ncomponents
        FFTPhi_tmp = FFTPhi_tmp + FFTPhysics3D.FPLaplace{n,m}.*FFTGPE_Phi{m}; % Applying the Thomas-Fermi preconditioner
    end
    Phi_out(:,:,(1+(n-1)*FFTGeometry3D.Nz):(n*FFTGeometry3D.Nz)) = ifftn(FFTPhi_tmp); % Storing the wave function with the operators and the preconditionner applied
end
