...
function [Phi_out] = operator_Full_RSP_FPThomasFermi3d(Phi_in, Psi, FFTPsi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.TimeDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
            GPE_Phi{n} = GPE_Phi{n} + ifftn(FFTPhysics3D.TimeDispersionImp{n,m}.*fftn(Phi{m}))/2; % Computing the wave function with the dispersion operator applied
        end 
    end
    ...
    GPE_Phi{n} = GPE_Phi{n} + ifftn((FFTPhysics3D.StochasticDispersion{n,n}/2 + FFTPhysics3D.Dispersion{n,n}/2 + FFTPhysics3D.TimeDispersionImp{n,n}/2).*fftn(Phi{n})); % Computing the wave function with the Laplacian operator applied
end
