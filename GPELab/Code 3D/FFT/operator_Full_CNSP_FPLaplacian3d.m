%% Applying the operators corresponding to the implicit part of the CNSP-CNFG scheme with the Laplace preconditioner
%% INPUTS:
...
function [Phi_out] = operator_Full_CNSP_FPLaplacian3d(Phi_in, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.Nonlinearity_function_Index{n}
        GPE_Phi = GPE_Phi + FFTPhysics3D.Beta*FFTPhysics3D.Nonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the non-local nonlinearity is non null
    for m = FFTPhysics3D.FFTNonlinearity_function_Index{n}
        GPE_Phi = GPE_Phi + FFTPhysics3D.Beta*FFTPhysics3D.FFTNonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the non-local nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics3D.TimePotential_function_Index{n}
        GPE_Phi = GPE_Phi + FFTPhysics3D.TimePotential{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
...
for n = 1:Method.Ncomponents
    ...
    Phi_out(:,:,(1+(n-1)*FFTGeometry3D.Nz):(n*FFTGeometry3D.Nz)) = Phi{n} + ifftn(FFTPhi_tmp); % Storing the wave function with the operators and the preconditionner applied
end
