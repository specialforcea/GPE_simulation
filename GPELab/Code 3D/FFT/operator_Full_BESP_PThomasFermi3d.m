%% Applying the operators corresponding to the implicit part of the BESP-CNFG scheme with the Laplace preconditionner
%% INPUTS:
...
function [Phi_out] = operator_Full_BESP_PThomasFermi3d(Phi_in, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.Dispersion_function_Index{n}
        ...
        if (m ~= n)
        GPE_Phi = GPE_Phi + ifftn(FFTPhysics3D.Dispersion{n,m}.*fftn(Phi{m})); % Computing the wave function with the dispersion operator applied
        end
    end
    ...
    GPE_Phi = GPE_Phi + ifftn(FFTPhysics3D.Dispersion{n,n}.*fftn(Phi{n})); % Computing the wave function with the Laplacian operator applied
    GPE_Phi = Phi{n} + 1./(1/Method.Deltat + FFTPhysics3D.Potential{n,n} + FFTPhysics3D.TimePotential{n,n} + FFTPhysics3D.Beta*FFTPhysics3D.Nonlinearity{n,n} + FFTPhysics3D.Beta*FFTPhysics3D.FFTNonlinearity{n,n}).*GPE_Phi; % Applying the Thomas-Fermi preconditionner
