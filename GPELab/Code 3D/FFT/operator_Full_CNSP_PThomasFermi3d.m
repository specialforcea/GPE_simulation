...
function [Phi_out] = operator_Full_CNSP_PThomasFermi3d(Phi_in, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = GPE_Phi + ifftn( FFTPhysics3D.Dispersion{n,n}/2.*fftn(Phi{n})); % Computing the wave function with the Laplacian operator applied
    GPE_Phi = Phi{n} + 1./(1/Method.Deltat + FFTPhysics3D.Potential{n,n}/2 + FFTPhysics3D.TimePotential{n,n}/2 + FFTPhysics3D.Beta*FFTPhysics3D.Nonlinearity{n,n}/2 + FFTPhysics3D.Beta*FFTPhysics3D.FFTNonlinearity{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
