...
function [Phi_out] = rhs_Full_RSP_FPLaplacian3d(Phi_in, Psi, FFTPsi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.Dispersion_function_Index{n}
        ...
        if (m ~= n)
        GPE_Phi = GPE_Phi - ifft(FFTPhysics3D.Dispersion{n,m}.*fft(Phi{m}))/2;
        end
    end
    ...
    for m = FFTPhysics3D.TimeDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi - ifft(FFTPhysics3D.TimeDispersionExp{n,m}.*fft(Phi{m}))/2;
        end
    end
    ...
    for m = FFTPhysics3D.StochasticDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi - ifft(FFTPhysics3D.StochasticDispersion{n,m}.*fft(Phi{m}))/2;
        end
    end
    FFTGPE_Phi{n} = fftn(GPE_Phi) ...
       + (1/Method.Deltat - FFTPhysics3D.Dispersion{n,n}/2 - FFTPhysics3D.TimeDispersionExp{n,n}/2 - FFTPhysics3D.StochasticDispersion{n,n}/2).*fftn(Phi{n}); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
end

%% Applying the Laplace preconditioner
for n = 1:Method.Ncomponents
    FFTPhi = zeros(FFTGeometry3D.Ny,FFTGeometry3D.Nx,FFTGeometry3D.Nz);
    for m = 1:Method.Ncomponents
        FFTPhi = FFTPhi + FFTPhysics3D.FPLaplace{n,m}.*FFTGPE_Phi{m}; % Applying the Thomas-Fermi preconditioner
    end
    Phi_out(:,:,(1+(n-1)*FFTGeometry3D.Nz):(n*FFTGeometry3D.Nz)) = ifftn(FFTPhi); % Storing the wave function with the operators and the preconditionner applied
end
