...
function [Phi_out] = operator_Full_CNSP_PLaplacian3d(Phi_in, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = Phi{n} + ifftn((1/Method.Deltat + FFTPhysics3D.Dispersion{n,n}/2).^(-1).*fftn(GPE_Phi)); % Computing the IFFT of the wave function with the operators applied then apply the Laplace preconditioner
