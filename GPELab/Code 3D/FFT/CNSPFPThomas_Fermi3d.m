%% Computing the Full ThomasFermi preconditioner for the CNSP scheme
%% INPUTS:
...
function FPTHomasFermi = CNSPFPThomas_Fermi3d(Method, FFTPhysics3D, FFTGeometry3D)
%% Initialization of the potential and nonlinear matricial operators 
PotNLMat = cell(Method.Ncomponents);
for n = 1:Method.Ncomponents
    for m = 1:Method.Ncomponents
        PotNLMat{n,m} = (n == m)*1/Method.Deltat + FFTPhysics3D.Potential{n,m}/2 + FFTPhysics3D.TimePotential{n,m}/2 + FFTPhysics3D.Beta*FFTPhysics3D.Nonlinearity{n,m}/2 + FFTPhysics3D.Beta*FFTPhysics3D.FFTNonlinearity{n,m}/2;
    end
end
