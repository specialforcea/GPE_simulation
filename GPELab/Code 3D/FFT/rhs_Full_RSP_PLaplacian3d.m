%% Applying the operators corresponding to the explicit part of the RSP scheme with the Laplace preconditioner
%% INPUTS:
...
function [Phi_out] = rhs_Full_RSP_PLaplacian3d(Phi_in, Psi, FFTPsi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.FFTNonlinearity_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics3D.Beta*FFTPsi{n,m}.*Phi{m}/2; % Computing the wave function with the non-local nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics3D.TimePotential_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics3D.TimePotentialExp{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    % FOR each component where the stochastic potential is non null
    for m = FFTPhysics3D.StochasticPotential_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics3D.StochasticPotential{n,m}.*Phi{m}/2; % Computing the wave function with the stochastic potential operator applied
    end
    ...
    GPE_Phi = ifftn((1/Method.Deltat + FFTPhysics3D.Dispersion{n,n}/2 + FFTPhysics3D.TimeDispersionImp{n,n}/2 + FFTPhysics3D.StochasticDispersion{n,n}/2).^(-1).*fftn(GPE_Phi)); % Applying the Laplace preconditioner
    Phi_out(:,:,(1+(n-1)*FFTGeometry3D.Nz):(n*FFTGeometry3D.Nz)) = GPE_Phi; % Storing the wave function with the operators applied
end
