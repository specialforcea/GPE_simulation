...
function [Phi_out] = rhs_Full_CNSP_PThomasFermi3d(Phi_in, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.Nonlinearity_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics3D.Beta*FFTPhysics3D.FFTNonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    ...
    GPE_Phi = GPE_Phi + 1/Method.Deltat*Phi{n} ...
        - ifftn(FFTPhysics3D.Dispersion{n,n}/2.*fftn(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
    GPE_Phi = 1./(1/Method.Deltat + FFTPhysics3D.Potential{n,n}/2 + FFTPhysics3D.TimePotential{n,n}/2 + FFTPhysics3D.Beta*FFTPhysics3D.Nonlinearity{n,n}/2 + FFTPhysics3D.Beta*FFTPhysics3D.FFTNonlinearity{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
