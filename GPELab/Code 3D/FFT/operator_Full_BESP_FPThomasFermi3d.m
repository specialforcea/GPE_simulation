%% Applying the operators corresponding to the implicit part of the BESP-CNFG scheme with the Thomas-Fermi preconditionner
%% INPUTS:
...
function [Phi_out] = operator_Full_BESP_FPThomasFermi3d(Phi_in, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics3D.Gradientx_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} + FFTPhysics3D.Gradientx{n,m}.*Gradx_Phi{m}; % Computing the wave function with the gradient operators in the x direction applied
    end
    % FOR each component where the gradient in the y direction is non null
    for m = FFTPhysics3D.Gradienty_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} + FFTPhysics3D.Gradienty{n,m}.*Grady_Phi{m}; % Computing the wave function with the gradient operators in the y direction applied
    end
    % FOR each component where the gradient in the z direction is non null
    for m = FFTPhysics3D.Gradientz_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} + FFTPhysics3D.Gradientz{n,m}.*Gradz_Phi{m}; % Computing the wave function with the gradient operators in the z direction applied
    end
    % FOR each component where the dispersion is non null
    for m = FFTPhysics3D.Dispersion_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} + ifftn(FFTPhysics3D.Dispersion{n,m}.*fftn(Phi{m})); % Computing the wave function with the dispersion operator applied
    end
end
...
for n = 1:Method.Ncomponents
    for m = 1:Method.Ncomponents
        Phi{n} = Phi{n} + FFTPhysics3D.FPThomasFermi{n,m}.*GPE_Phi{m}; % Applying the Thomas-Fermi preconditioner
    end
    Phi_out(:,:,(1+(n-1)*FFTGeometry3D.Nz):(n*FFTGeometry3D.Nz)) = Phi{n}; % Storing the wave function with the operators and the preconditionner applied
end
