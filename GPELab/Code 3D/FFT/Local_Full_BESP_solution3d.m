...
function [Phi, flag, relres, iter, resvec] = Local_Full_BESP_solution3d(Phi, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D)
...
%% Computing the wave functions after a single time step using Krylov iterative method for BESP-CNFG
...
if (strcmp(Method.Precond,'Laplace') == 1)
   ...
   [Phi_out, flag, relres, iter, resvec] = bicgstab(@(x)operator_Full_BESP_PLaplacian3d(x, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D),...
   LinearLaplace_preconditioner3d((1/Method.Deltat)*Phi_vect, Method, FFTGeometry3D, FFTPhysics3D), Method.Iterative_tol, Method.Iterative_maxit,[],[],Phi_vect);
...
elseif (strcmp(Method.Precond,'ThomasFermi') == 1)
   ...
   % Moreover, it uses the initial components' wave functions as initial vector.
   [Phi_out, flag, relres, iter, resvec] = bicgstab(@(x)operator_Full_BESP_PThomasFermi3d(x, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D),...
   TF_preconditioner3d((1/Method.Deltat)*Phi_vect, Method, FFTGeometry3D, FFTPhysics3D),Method.Iterative_tol, Method.Iterative_maxit,[],[],Phi_vect);
% ELSEIF one has chosen to use the ThomasFermi preconditioner
elseif (strcmp(Method.Precond,'FThomasFermi') == 1)
   ...
   % and the explicit part with the Thomas-Fermi preconditioner applied. 
   % Moreover, it uses the initial components' wave functions as initial vector.
   [Phi_out, flag, relres, iter, resvec] = bicgstab(@(x)operator_Full_BESP_FPThomasFermi3d(x, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D),...
   FTF_preconditioner3d((1/Method.Deltat)*Phi_vect, Method, FFTGeometry3D, FFTPhysics3D),Method.Iterative_tol, Method.Iterative_maxit,[],[],Phi_vect);
% ELSEIF one has chosen to use the ThomasFermi preconditioner
elseif (strcmp(Method.Precond,'FLaplace') == 1)
   ...
   % and the explicit part with the Thomas-Fermi preconditioner applied. 
   % Moreover, it uses the initial components' wave functions as initial vector.
   [Phi_out, flag, relres, iter, resvec] = bicgstab(@(x)operator_Full_BESP_FPLaplacian3d(x, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D),...
   FLP_preconditioner3d((1/Method.Deltat)*Phi_vect, Method, FFTGeometry3D, FFTPhysics3D),Method.Iterative_tol, Method.Iterative_maxit,[],[],Phi_vect);
...
elseif (strcmp(Method.Precond,'None') == 1)
   ...
   [Phi_out, flag, relres, iter, resvec] = bicgstab(@(x)operator_Full_BESP3d(x, Method, FFTGeometry3D, FFTPhysics3D, FFTOperators3D),...
