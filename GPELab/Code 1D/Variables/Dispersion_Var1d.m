...
%% INPUTS(OPTIONAL):
%%          Dispersion: Function or cell array of functions that depends on (FFTX) which will be multiplied by the wave function in the physical problem (function or cell array of functions)
...
function [Physics1D] = Dispersion_Var1d(Method, Physics1D, Dispersion, G)
%% Initializing the default potential
Default_Dispersion = @(FFTX) Physics1D.Delta*(FFTX.^2);
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                ...
                if (G(n,m)~= 0)
                    ...
                    if(n == m)
                    Physics1D.Dispersion_function{n,m} = @(FFTX)G(n,m)*Default_Dispersion(FFTX); % Storing the potential as the correspondant value of G at the index added by the default potential
        ...
        Physics1D.Dispersion_function_Index{n} = Dispersion_function_index; % Store the potential index for the 'm' index
    end
...
elseif (nargin == 4) && (iscell(Dispersion) == 0) && (isempty(Dispersion) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF the value of the matrix at the index is not zero
                if (G(n,m)~= 0)
                    Physics1D.Dispersion_function{n,m} = @(FFTX)G(n,m)*Dispersion(FFTX); % Storing the potential as the correspondant value of G at the index multiplied by the defined potential
                    Dispersion_function_index = [Dispersion_function_index,m]; % Add the 'm' index in the temporary potential index
                % ELSEIF the value of the matrix at the index is zero
                elseif (G(n,m) == 0)
                    Physics1D.Dispersion_function{n,m} = @(FFTX)0; % Storing zero in the potential function matrix
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                ...
                if (G(n,m)~= 0) && (isempty(Dispersion{n,m}) == 0)
                    Physics1D.Dispersion_function{n,m} = @(FFTX)G(n,m)*Dispersion{n,m}(FFTX); % Storing the potential as the correspondant value of G at the index multiplied by the defined potential
                    Dispersion_function_index = [Dispersion_function_index,m]; % Add the 'm' index in the temporary potential index
...
elseif (nargin == 3) && (iscell(Dispersion) == 0) && (isempty(Dispersion) == 1)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF it is a diagonal term
                if (n == m)
                    Physics1D.Dispersion_function{n,m} = @(FFTX)Default_Dispersion(FFTX); % Storing the potential as the default potential
                    Dispersion_function_index = [Dispersion_function_index,m]; % Add the 'm' index in the temporary potential index
...
elseif (nargin == 3) && (iscell(Dispersion) == 0) && (isempty(Dispersion) == 0)
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
                % IF it is a diagonal term
                if (n == m)
                    Physics1D.Dispersion_function{n,m} = @(FFTX)Dispersion(FFTX); % Storing the potential as the defined potential
                    Dispersion_function_index = [Dispersion_function_index,m]; % Add the 'm' index in the temporary potential index
                % ELSEIF it is an extradiagonal term
                elseif (n ~= m)
                    Physics1D.Dispersion_function{n,m} = @(FFTX)0; % Storing zero in the potential function matrix
    ...
    for n = 1:Method.Ncomponents
        ...
        for m = 1:Method.Ncomponents
            ...
            if (isempty(Dispersion{n,m}) == 0)
                Physics1D.Dispersion_function{n,m} = @(FFTX)Dispersion{n,m}(FFTX); % Storing the potential as the defined potential
                Dispersion_function_index = [Dispersion_function_index,m]; % Add the 'm' index in the temporary potential index
