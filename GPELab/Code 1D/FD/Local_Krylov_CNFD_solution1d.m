...
function [Phi, flag, relres, iter, resvec] = Local_Krylov_CNFD_solution1d(Phi, Method, FDGeometry1D, FDPhysics1D, FDOperators1D)
...
% part of the BEFD scheme and the explicit part. Moreover, it uses the 
% initial components' wave functions as initial vector.
[Phi_out, flag, relres, iter, resvec] = bicgstab(((1/Method.Deltat)*FDOperators1D.Id + FDOperators1D.Linear_Operators/2 + FDPhysics1D.Beta*FDOperators1D.Nonlinearity_matrix/2 + FDOperators1D.TimePotential_matrix/2),...
((1/Method.Deltat)*FDOperators1D.Id - FDOperators1D.Linear_Operators/2 - FDPhysics1D.Beta*FDOperators1D.Nonlinearity_matrix/2 - FDOperators1D.TimePotential_matrix/2)*Phi_vect, Method.Iterative_tol, Method.Iterative_maxit,[],[],Phi_vect);

%% Reshaping vector output from the iterative method as matrix
% FOR each component
for n = 1:Method.Ncomponents
    Phi{n} = reshape(Phi_out((1+(n-1)*FDGeometry1D.Nx):(n*FDGeometry1D.Nx)),FDGeometry1D.Nx,1); % Storing the components' wave functions back in a cell array
end