...
%% OUTPUTS:
%%          Phi_final: Ground states' wave functions computated with the BEFD method (cell array)
%%          Outputs: Different outputs computed during the computation of the ground states (structure) (see OutputsINI_Var1D.m and FFTOutputs_Var1d.m)
%% FUNCTIONS USED:
...
%%          Local_Krylov_BEFD_solution1d: To compute the CNFG on a single time step using the iterative BEFD method (line 63)
%%          Local_Direct_BEFD_solution1d: To compute the CNFG on a single time step using the direct BEFD method (line 69)
...
function [Phi_final, Outputs] = BEFD_CNGF1d(Phi_0, Method, Geometry1D, Physics1D, Outputs, Print, Figure)
...
while (Method.EvolutionCriterion > Method.Stop_crit{2}*Method.Deltat)&&(Method.Iterations<Method.Max_iter)
    ...
    Energy = Energy_GPE_Diff1d(FDPhi, Method, FDGeometry1D, FDPhysics1D, FDOperators1D); % Computing the energy of each wave function
