%% Applying the operators corresponding to the implicit part of the RSP-DS scheme with the Thomas-Fermi preconditionner
%% INPUTS:
...
function [Phi_out] = operator_Full_RSP_PThomasFermi1d(Phi_in, Psi, FFTPsi, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics1D.StochasticPotential_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + FFTPhysics1D.StochasticPotential{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
        end
    end
    ...
    GPE_Phi = GPE_Phi + ifft((FFTPhysics1D.Dispersion{n,n}/2 + FFTPhysics1D.StochasticDispersion{n,n}/2).*fft(Phi{n})); % Computing the wave function with the Laplacian operator applied
    GPE_Phi = Phi{n} + 1./(1/Method.Deltat + FFTPhysics1D.Potential{n,n}/2 + FFTPhysics1D.TimePotentialImp{n,n}/2 + FFTPhysics1D.StochasticPotential{n,n}/2 + FFTPhysics1D.Beta*Psi{n,n}/2 + FFTPhysics1D.Beta*FFTPsi{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
