%% Applying the operators corresponding to the explicit part of the RSP-CNFG scheme with the Thomas-Fermi preconditioner
%% INPUTS:
...
function [Phi_out] = rhs_Full_RSP_FPThomasFermi1d(Phi_in, Psi, FFTPsi, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics1D.Potential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics1D.Potential{n,m}.*Phi{m}/2; % Computing the wave function with the potential operator applied
    end
    % FOR each component where the nonlinearity is non null
    for m = FFTPhysics1D.Nonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics1D.Beta*Psi{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the non-local nonlinearity is non null
    for m = FFTPhysics1D.FFTNonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics1D.Beta*FFTPsi{n,m}.*Phi{m}/2; % Computing the wave function with the non-local nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics1D.TimePotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics1D.TimePotentialExp{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    % FOR each component where the stochastic potential is non null
    for m = FFTPhysics1D.StochasticPotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics1D.StochasticPotential{n,m}.*Phi{m}/2; % Computing the wave function with the stochastic potential operator applied
    end
    % FOR each component where the stochastic dispersion is non null
    for m = FFTPhysics1D.StochasticDispersion_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - iff(FFTPhysics1D.StochasticDispersion{n,m}.*fft(Phi{m}))/2;
    end
    ...
    for m = FFTPhysics1D.TimeDispersion_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - ifft(FFTPhysics1D.TimeDispersionExp{n,m}.*fft(Phi{m}))/2;
    end
    % FOR each component where the stochastic dispersion is non null
    for m = FFTPhysics1D.StochasticDispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi{n} = GPE_Phi{n} - ifft(FFTPhysics1D.StochasticDispersion{n,m}.*fft(Phi{m}))/2;
        end
    end
    GPE_Phi{n} = GPE_Phi{n} + 1/Method.Deltat*Phi{n} ...
        + ifft((- FFTPhysics1D.Dispersion{n,n}/2 - FFTPhysics1D.StochasticDispersion{n,n}/2).*fft(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
end

%% Applying the full Thomas Fermi preconditioner
% FOR each component
for n = 1:Method.Ncomponents
    Phi = zeros(FFTGeometry1D.Nx,1); % Initialization of the wave function
    % FOR each component
    for m = 1:Method.Ncomponents
        Phi = Phi + FFTPhysics1D.FPThomasFermi{n,m}.*GPE_Phi{m}; % Applying the Thomas-Fermi preconditioner
    end
    Phi_out((1+(n-1)*FFTGeometry1D.Nx):(n*FFTGeometry1D.Nx)) = Phi; % Storing the wave function with the operators and the preconditioner applied
end
