%% Applying the operators corresponding to the implicit part of the CNSP-CNFG scheme with the Thomas-Fermi preconditionner
%% INPUTS:
...
function [Phi_out] = operator_Full_CNSP_PThomasFermi1d(Phi_in, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics1D.Nonlinearity_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + FFTPhysics1D.Beta*FFTPhysics1D.Nonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
        end
    end
    ...
    for m = FFTPhysics1D.FFTNonlinearity_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + FFTPhysics1D.Beta*FFTPhysics1D.FFTNonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the non-local nonlinearity operator applied
        end
    end
    ...
    for m = FFTPhysics1D.TimePotential_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + FFTPhysics1D.TimePotential{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
        end
    end
    ...
    GPE_Phi = GPE_Phi + ifft(FFTPhysics1D.Dispersion{n,n}/2.*fft(Phi{n})); % Computing the wave function with the Laplacian operator applied
    GPE_Phi = Phi{n} + 1./(1/Method.Deltat + FFTPhysics1D.Potential{n,n}/2 + FFTPhysics1D.TimePotential{n,n}/2 + FFTPhysics1D.Beta*FFTPhysics1D.Nonlinearity{n,n}/2 + FFTPhysics1D.Beta*FFTPhysics1D.FFTNonlinearity{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
