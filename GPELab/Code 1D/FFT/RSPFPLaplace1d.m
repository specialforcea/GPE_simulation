...
function FPLaplace = RSPFPLaplace1d(Method, FFTPhysics1D, FFTGeometry1D)
%% Initialization of the potential and nonlinear matricial operators 
DispMat = cell(Method.Ncomponents);
for n = 1:Method.Ncomponents
    for m = 1:Method.Ncomponents
        DispMat{n,m} = (n == m)*1/Method.Deltat + FFTPhysics1D.Dispersion{n,m}/2 + FFTPhysics1D.StochasticDispersion{n,m}/2;
    end
    for m = FFTPhysics1D.TimeDispersion_function_Index{n}
        DispMat{n,m} = DispMat{n,m} + FFTPhysics1D.TimeDispersionImp{n,m}/2;
    end
end
