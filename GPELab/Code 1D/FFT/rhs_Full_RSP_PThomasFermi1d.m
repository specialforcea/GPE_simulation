...
function [Phi_out] = rhs_Full_RSP_PThomasFermi1d(Phi_in, Psi, FFTPsi, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics1D.TimePotential_function_Index{n}
        GPE_Phi = GPE_Phi - FFTPhysics1D.TimePotentialExp{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    ...
    for m = FFTPhysics1D.TimeDispersion_function_Index{n}
        GPE_Phi = GPE_Phi - ifft(FFTPhysics1D.TimeDispersionExp{n,m}.*fft(Phi{m}))/2;
    end
    ...
    GPE_Phi = 1./(1/Method.Deltat + FFTPhysics1D.Potential{n,n}/2+ FFTPhysics1D.TimePotentialImp{n,n}/2 + FFTPhysics1D.StochasticPotential{n,n}/2 + FFTPhysics1D.Beta*Psi{n,n}/2 + FFTPhysics1D.Beta*FFTPsi{n,n}/2).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
