...
function [Phi_out] = operator_Full_BESP_PThomasFermi1d(Phi_in, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics1D.Potential_function_Index{n}
        ...
        if (m ~= n)
        GPE_Phi = GPE_Phi + FFTPhysics1D.Potential{n,m}.*Phi{m}; % Computing the wave function with the potential operator applied
        end
    end
    ...
    for m = FFTPhysics1D.Nonlinearity_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + FFTPhysics1D.Beta*FFTPhysics1D.Nonlinearity{n,m}.*Phi{m}; % Computing the wave function with the nonlinearity operator applied
        end
    end
    ...
    for m = FFTPhysics1D.FFTNonlinearity_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + FFTPhysics1D.Beta*FFTPhysics1D.FFTNonlinearity{n,m}.*Phi{m}; % Computing the wave function with the non-local nonlinearity operator applied
        end
    end
    ...
    for m = FFTPhysics1D.TimePotential_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + FFTPhysics1D.TimePotential{n,m}.*Phi{m}; % Computing the wave function with the time-dependent potential operator applied
        end
    end
    ...
    for m = FFTPhysics1D.Dispersion_function_Index{n}
        % IF it is an extradiagonal term
        if (m ~= n)
        GPE_Phi = GPE_Phi + ifft(FFTPhysics1D.Dispersion{n,m}.*fft(Phi{m}));
        end
    end
    GPE_Phi = GPE_Phi + ifft(FFTPhysics1D.Dispersion{n,n}.*fft(Phi{n})); % Computing the wave function with the Laplacian operator applied
    GPE_Phi = Phi{n} + 1./(1/Method.Deltat + FFTPhysics1D.Potential{n,n}+ FFTPhysics1D.TimePotential{n,n} + FFTPhysics1D.Beta*FFTPhysics1D.Nonlinearity{n,n} + FFTPhysics1D.Beta*FFTPhysics1D.FFTNonlinearity{n,n}).*GPE_Phi; % Applying the Thomas-Fermi preconditioner
    Phi_out((1+(n-1)*FFTGeometry1D.Nx):(n*FFTGeometry1D.Nx)) = GPE_Phi; % Storing the wave function with the operators and the preconditioner applied
end
