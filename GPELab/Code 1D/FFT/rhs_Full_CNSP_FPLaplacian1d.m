...
function [Phi_out] = rhs_Full_CNSP_FPLaplacian1d(Phi_in, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = GPE_Phi + ifft((1/Method.Deltat+ FFTPhysics1D.Dispersion{n,n}/2).*fft(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
    FFTGPE_Phi{n} = fft(GPE_Phi);
end
...
for n = 1:Method.Ncomponents
    ...
    for m = 1:Method.Ncomponents
        FFTPhi = FFTPhi + FFTPhysics1D.FPLaplace{n,m}.*FFTGPE_Phi{m}; % Applying the Laplace spreconditioner
    end
    Phi_out((1+(n-1)*FFTGeometry1D.Nx):(n*FFTGeometry1D.Nx)) = ifft(FFTPhi); % Storing the wave function with the operators and the preconditioner applied
end
