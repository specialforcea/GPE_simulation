...
%% INPUTS:
%%          Phi_in: Initial component functions (cell array)
...
%% OUTPUT:
%%          Phi_out: Component functions with the operators applied (cell array)

function [Phi_out] = LinearLaplace_preconditioner1d(Phi_in, Method, FFTGeometry1D, FFTPhysics1D)
...
Phi = ifft((1/Method.Deltat + FFTPhysics1D.Dispersion{n,n}).^(-1).*fft(Phi)); % Applying the Laplace preconditoner
Phi_out((1+(n-1)*FFTGeometry1D.Nx):(n*FFTGeometry1D.Nx)) = Phi; % Storing the wave function of a component with the Laplace preconditoner applied
end
