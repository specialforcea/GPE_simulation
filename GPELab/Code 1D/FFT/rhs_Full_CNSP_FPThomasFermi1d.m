%% Applying the operators corresponding to the explicit part of the CNSP-CNFG scheme with the full Thomas-Fermi preconditioner
%% INPUTS:
...
function [Phi_out] = rhs_Full_CNSP_FPThomasFermi1d(Phi_in, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics1D.Nonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics1D.Beta*FFTPhysics1D.Nonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the nonlinearity operator applied
    end
    % FOR each component where the non-local nonlinearity is non null
    for m = FFTPhysics1D.FFTNonlinearity_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics1D.Beta*FFTPhysics1D.FFTNonlinearity{n,m}.*Phi{m}/2; % Computing the wave function with the non-local nonlinearity operator applied
    end
    % FOR each component where the time-dependent potential is non null
    for m = FFTPhysics1D.TimePotential_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} - FFTPhysics1D.TimePotential{n,m}.*Phi{m}/2; % Computing the wave function with the time-dependent potential operator applied
    end
    ...
    GPE_Phi{n} = GPE_Phi{n} + 1/Method.Deltat*Phi{n} ...
        + ifft((- FFTPhysics1D.Dispersion{n,n}/2).*fft(Phi{n})); % Computing the wave function with the potential, nonlinear and Laplacian operators applied
end
