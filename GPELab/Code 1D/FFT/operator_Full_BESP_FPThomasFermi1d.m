...
function [Phi_out] = operator_Full_BESP_FPThomasFermi1d(Phi_in, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics1D.Gradientx_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} + FFTPhysics1D.Gradientx{n,m}.*Gradx_Phi{m}; % Computing the wave function with the gradient operators in the x direction applied
    end
    ...
    for m = FFTPhysics1D.Dispersion_function_Index{n}
        ...
        if (m ~= n)
        GPE_Phi{n} = GPE_Phi{n} + ifft(FFTPhysics1D.Dispersion{n,m}.*fft(Phi{m}));
        end
    end
    GPE_Phi{n} = GPE_Phi{n} + ifft(FFTPhysics1D.Dispersion{n,n}.*fft(Phi{n})); % Computing the wave function with the Laplacian operator applied
end
