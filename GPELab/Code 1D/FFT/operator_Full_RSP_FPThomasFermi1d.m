%% Applying the operators corresponding to the implicit part of the RSP-DS scheme with the full Thomas-Fermi preconditionner
%% INPUTS:
...
function [Phi_out] = operator_Full_RSP_FPThomasFermi1d(Phi_in, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    for m = FFTPhysics1D.TimeDispersion_function_Index{n}
        GPE_Phi{n} = GPE_Phi{n} + ifft(FFTPhysics1D.TimeDispersionImp{n,m}.*fft(Phi{m}))/2;
    end
    ...
    GPE_Phi{n} = GPE_Phi{n} + ifft((FFTPhysics1D.Dispersion{n,n}/2 + FFTPhysics1D.StochasticDispersion{n,n}/2).*fft(Phi{n})); % Computing the wave function with the Laplacian operator applied
end
