%% Applying the operators corresponding to the explicit part of the RSP-CNFG scheme with the Laplace preconditioner
%% INPUTS:
...
function [Phi_out] = rhs_Full_RSP_PLaplacian1d(Phi_in, Psi, FFTPsi, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    GPE_Phi = ifft((1/Method.Deltat + FFTPhysics1D.Dispersion{n,n}/2 + FFTPhysics1D.StochasticDispersion{n,n}/2).^(-1).*fft(GPE_Phi)); % Applying the Laplace preconditioner
