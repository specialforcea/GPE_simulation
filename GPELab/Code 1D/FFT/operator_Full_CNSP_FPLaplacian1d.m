%% Applying the operators corresponding to the implicit part of the CNSP-CNFG scheme with the full Laplace preconditionner
%% INPUTS:
...
function [Phi_out] = operator_Full_CNSP_FPLaplacian1d(Phi_in, Method, FFTGeometry1D, FFTPhysics1D, FFTOperators1D)
...
for n = 1:Method.Ncomponents
    ...
    Phi_out((1+(n-1)*FFTGeometry1D.Nx):(n*FFTGeometry1D.Nx)) = Phi{n} + ifft(FFTPhi); % Storing the wave function with the operators and the preconditioner applied
end
