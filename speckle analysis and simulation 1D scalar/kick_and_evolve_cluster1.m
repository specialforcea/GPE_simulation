...
for mk=30:30
    ...
    rand_row = randi([1 8193],1,40);
    for j=1:20

        %speckle_row = speckle(rand_row(j),:);
        speckle_row = [speckle(rand_row(j),:) speckle(rand_row(j+20),2:end)];
...
%             prof_evo(mk,j,i) = sqrt(integr((X-mean_prof).^2.*sq(phi_1(1,:)),Nx,deltax));
            ...
            if i==200
                final_phi_200(j,:) = reshape(phi(1,:),[1,Nx]);
            end
        ...
        final_phi(j,:) = reshape(phi(1,:),[1,Nx]);
