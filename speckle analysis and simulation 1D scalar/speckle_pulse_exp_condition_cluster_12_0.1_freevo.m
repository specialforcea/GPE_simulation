...
mom_evo = zeros(20,256);
prof_evo = zeros(20,256);
mean_m = zeros(20,256);
mean_p = zeros(20,256);
poten = zeros(20,6);
final_phi = zeros(20,52,Nx);

for k=1:10
    speckle_ = speckle*(k-1)*1.6;
    save_path = strcat('simulation_results/09032020speckle_pulse_exp_condition_cluster_12_0.1_freevo/',num2str((k-1)*200),'/');
    ...
    for j=1:20
        ...
        for i=7:256
            ...
            phi = phi_1;
            if mod(i-6,5)==0
                final_phi(j,int32((i+4)/5),:) = reshape(phi(1,:),[1,1,Nx]);
            end
